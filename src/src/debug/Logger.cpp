...
{
    void Logger::log(const std::string& message, std::string specificFile, Logger::severity level)
    {
    ...
    void Logger::_logFile(const std::string& message, const std::string& filepath, Logger::severity level)
    {
        if (_mode == RELEASE_MODE && level == DEBUG)
            return;
